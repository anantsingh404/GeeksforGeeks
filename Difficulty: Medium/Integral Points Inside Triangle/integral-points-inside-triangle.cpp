...
class Solution {
  public:
    long long int InternalCount(long long int p[], long long int q[],
        ...
        long long int x=__gcd(abs(p[0]-q[0]),abs(p[1]-q[1]));
         long long int y=__gcd(abs(q[0]-r[0]),abs(q[1]-r[1]));
          long long int z=__gcd(abs(r[0]-p[0]),abs(r[1]-p[1]));
          long long int pq=x+y+z;
          return area-pq/2+1;
